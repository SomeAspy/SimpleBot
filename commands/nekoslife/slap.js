...
export const description='slap a person!';
export function execute(message,args,client){
    let user;
    let text='You can slap another person by mentioning them!'
    if(args[0]){
        user=mentionToUser(args[0],client);
        if(!user){return message.reply('Invalid user!')}else{
            text=`${message.author.username} slaps ${user.username}!`
        }}
    async function returnNeko(){
        const nekoOut=await neko.sfw.slap()
