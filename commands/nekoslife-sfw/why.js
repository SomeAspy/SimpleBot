...
export const description='why a person!';
export function execute(message,args){
    let user;
    let text='You can why another person by mentioning them!'
    if(args[0]){
        user=mentionToUser(args[0]);
        if(!user){return message.reply('Invalid user!')}else{
            text=`${message.author.username} whys ${user.username}!`
        }}
    async function returnNeko(){
        const nekoOut=await neko.sfw.why()
