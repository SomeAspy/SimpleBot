...
export const name='lesbian';
export const description='Get a lesbian pic!';
...
export function execute(message,args){
    ...
    if(args[0]){
        ...
        if(!user){return message.reply('Invalid user!')}else{
            text=`${message.author.username} lesbians ${user.username}!`
        }}
    async function returnNeko(){
        const nekoOut=await neko.nsfw.lesbian()
