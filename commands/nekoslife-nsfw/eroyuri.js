...
export const name='eroyuri';
export const description='Get a eroyuri pic!';
...
export function execute(message,args){
    ...
    if(args[0]){
        ...
        if(!user){return message.reply('Invalid user!')}else{
            text=`${message.author.username} eroyuris ${user.username}!`
        }}
    async function returnNeko(){
        const nekoOut=await neko.nsfw.eroYuri()
