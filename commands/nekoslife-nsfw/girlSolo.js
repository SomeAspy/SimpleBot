...
export const name='girtSolo';
export const description='Get a girtSolo pic!';
...
export function execute(message,args){
    ...
    if(args[0]){
        ...
        if(!user){return message.reply('Invalid user!')}else{
            text=`${message.author.username} girtSolos ${user.username}!`
        }}
    async function returnNeko(){
        const nekoOut=await neko.nsfw.girtSolo()
