...
export const name='erofeet';
export const description='Get a erofeet pic!';
...
export function execute(message,args){
    ...
    if(args[0]){
        ...
        if(!user){return message.reply('Invalid user!')}else{
            text=`${message.author.username} erofeets ${user.username}!`
        }}
    async function returnNeko(){
        const nekoOut=await neko.nsfw.eroFeet()
