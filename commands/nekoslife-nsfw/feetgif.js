...
export const name='feetgif';
export const description='Get a feetgif pic!';
...
export function execute(message,args){
    ...
    if(args[0]){
        ...
        if(!user){return message.reply('Invalid user!')}else{
            text=`${message.author.username} feetgifs ${user.username}!`
        }}
    async function returnNeko(){
        const nekoOut=await neko.nsfw.feetGif()
