...
export const name='cumsluts';
export const description='Get a cumsluts pic!';
...
export function execute(message,args){
    ...
    if(args[0]){
        ...
        if(!user){return message.reply('Invalid user!')}else{
            text=`${message.author.username} cumslutss ${user.username}!`
        }}
    async function returnNeko(){
        const nekoOut=await neko.nsfw.cumsluts()
