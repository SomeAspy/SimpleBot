...
export const name='erokemonomimi';
export const description='Get a erokemonomimi pic!';
...
export function execute(message,args){
    ...
    if(args[0]){
        ...
        if(!user){return message.reply('Invalid user!')}else{
            text=`${message.author.username} erokemonomimis ${user.username}!`
        }}
    async function returnNeko(){
        const nekoOut=await neko.nsfw.eroKemonomimi()
