...
export const name='randomhentaigif';
export const description='Get a random Hentai GIF!';
...
export function execute(message,args){
    ...
    if(args[0]){
        ...
        if(!user){return message.reply('Invalid user!')}else{
            text=`${message.author.username} HentaiGifs ${user.username}!`
        }}
    async function returnNeko(){
        const nekoOut=await neko.nsfw.randomHentaiGif()
