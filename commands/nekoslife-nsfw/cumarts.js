...
export const name='cumarts';
export const description='Get a cumarts pic!';
...
export function execute(message,args){
    ...
    if(args[0]){
        ...
        if(!user){return message.reply('Invalid user!')}else{
            text=`${message.author.username} cumartss ${user.username}!`
        }}
    async function returnNeko(){
        const nekoOut=await neko.nsfw.cumArts()
