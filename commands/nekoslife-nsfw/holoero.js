...
export const name='holoero';
export const description='Get a holoero pic!';
...
export function execute(message,args){
    ...
    if(args[0]){
        ...
        if(!user){return message.reply('Invalid user!')}else{
            text=`${message.author.username} holoeros ${user.username}!`
        }}
    async function returnNeko(){
        const nekoOut=await neko.nsfw.holoEro()
